...
for sim in tqdm(range(1000)):
    ...
    lb = 0.01
    ub = 0.99
    upper_band = alpha_center + 0.05
    lower_band = alpha_center - 0.05
    ...
    for day in t_list[1:-1]: #Gamma can only be calculated till two days before expiry since it's the second order derivative
        ...
        if alpha >= ub:
            ...
            upper_band = 0.01
            lower_band = 0
        if alpha > lower_band and alpha < upper_band:
...
plt.ylabel('count')
plt.title('P/L Distribution, Move-based Delta Gamma Hedge')
plt.savefig('Profit Distribition, Move-based Delta Gamma Hedge.png')
plt.show()
